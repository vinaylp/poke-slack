...
class MessagePoller {
  ...
  async pollChannel(channelId) {
    ...
    // Enrich and format messages
    const formattedMessages = await Promise.all(
      rawMessages.map(msg => this._formatMessage(msg, channel))
    );
   ...
   * Handles pagination if there are more than 1000 messages.
   *
   * @private
  ...
  async _fetchMessages(channelId, sinceTimestamp) {
    try {
      ...
      // This returns messages in reverse chronological order (newest first)
      const result = await this.slack.client.conversations.history({
        channel: channelId,
        oldest: sinceTimestamp, // Only fetch messages after this timestamp
        limit: 1000, // Maximum allowed by Slack
        inclusive: false // Don't include the message at oldest timestamp (already processed)
      });

      // Reverse to get chronological order (oldest first)
      const messages = result.messages.reverse();

      // Filter out bot messages and system messages if desired
      // For now, we'll include everything
      const filteredMessages = messages.filter(msg => {
        // Skip messages without text (e.g., some system events)
